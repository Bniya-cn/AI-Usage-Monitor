...
public class KimiApiCollector extends BaseCollector {
    ...
    public List<UsageRecord> collect() throws Exception {
        ...
        if (key == null || key.trim().isEmpty()) {
            throw new IllegalArgumentException("未配置 KIMI API Key，请在设置中添加。");
        }
        ...
        try {
            ...
            if (e.getMessage() != null && (e.getMessage().contains("401") || e.getMessage().contains("403"))) {
                throw new IllegalArgumentException("KIMI API Key 无效或无权限。");
            }
            throw e;
        }
        ...
        extra.addProperty("note", "已校验 KIMI API Key。当前版本不抓取网页用量。");
